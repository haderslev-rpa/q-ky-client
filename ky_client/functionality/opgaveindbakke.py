...
class OpgaveindbakkeClient:
    ...
    async def hent_opgaver(
        ...
        - ``Opgave-Id``
        - ``Original URL``
        - ``URL``: korrigeret med ``/ky-fagsystem`` og opgave-ID

        Hvis den fundne URL eksempelvis er::

            https://fs0510.fs.kommunernesydelsessystem.dk/
            entitet/overblik?cb

        bliver det offentlige URL-felt eksempelvis::

            https://fs0510.fs.kommunernesydelsessystem.dk/
            ky-fagsystem/entitet/overblik?cb&opgaveId=123

        ``Original URL`` bevarer den URL, som blev fundet i tabellen.

        Hjælpefeltet ``_Alle URLer`` bruges kun internt og fjernes fra
        outputtet.
        ...
        package_name = self._normalise(
            opgavepakke
        )

        if not package_name:
            raise ValueError(
                "Navnet på opgavepakken må ikke være tomt."
            )
        ...
        await self._wait_for_dropdown_open(
            dropdown
        )
        ...
        self.forventet_antal = self._extract_count(
            selected_text
        )
        ...
        table = await self._wait_for_table_stable(
            table_frame
        )

        self._last_table = table

        # _read_table() kalder _ret_opgave_urls().
        #
        # Det betyder, at rows allerede indeholder den korrigerede
        # URL med /ky-fagsystem, når denne metode returnerer.
        rows = await self._read_table(
            table
        )

        self._last_rows = await self._get_visible_data_rows(
            table
        )
    ...
    async def aabn_foerste_opgave(self) -> None:
        ...
        if not self._last_rows:
            raise OpgaveindbakkeError(
                "Der er ingen tabelrækker at åbne. "
                "Kald hent_opgaver() først."
            )
    ...
    async def _wait_for_dropdown_open(
        ...
        while elapsed_ms < TABLE_TIMEOUT_MS:
            ...
            try:
                expanded = await dropdown.get_attribute(
                    "aria-expanded"
                )
    ...
    async def _get_current_table_state(
        self,
    ) -> str | None:
        ...
        for frame in self._page.frames:
            try:
                ...
                return await self._get_table_state(
                    table
                )
        ...
        while elapsed_ms < TABLE_TIMEOUT_MS:
            ...
            for frame in self._page.frames:
                try:
                    ...
                    new_state = await self._get_table_state(
                        table
                    )
    ...
    async def _read_table(
        ...
        return self._ret_opgave_urls(
            rows
        )

    async def _get_visible_data_rows(
        self,
        table: Locator,
    ) -> list:
    ...
    def _ret_opgave_urls(
        ...
        """Returnér Original URL og en korrekt navigerbar URL.

        ``Original URL`` bevarer den URL, som blev fundet i KY-tabellen.

        ``URL`` korrigeres, så stien indeholder ``/ky-fagsystem``.
        Derefter tilføjes ``opgaveId``, hvis parameteren mangler.
        """
        ...
        for row_number, row in enumerate(
            ...
            if original_url:
                cleaned_url = self._clean_task_url(
                    original_url
                )

                corrected_url = self._ensure_ky_context_path(
                    cleaned_url
                )

                if task_id:
                    final_url = self._build_task_url(
                        corrected_url,
                        task_id,
                    )
                else:
                ...
                if final_url != original_url:
                    logger.info(
                        "Opgave-URL blev korrigeret. "
                        "Række=%d, Opgave-Id=%r, "
                        "Original URL=%r, URL=%r.",
                        row_number,
                        task_id or None,
                        original_url,
                        final_url,
                ...
                logger.warning(
                    "Der blev ikke fundet en URL på opgaverække %d. "
                    "Opgave-Id=%r.",
                    row_number,
                    task_id or None,
                )
    ...
    @staticmethod
    def _ensure_ky_context_path(
        url: str,
    ) -> str:
        """Sikr, at URL-stien indeholder ``/ky-fagsystem``.
        ...
        Host, queryparametre og fragment bevares.
        """
        ...
        if not parsed_url.scheme:
            raise ValueError(
                "Opgave-URL'en mangler scheme. "
                f"URL={url!r}."
            )

        if not parsed_url.netloc:
            raise ValueError(
                "Opgave-URL'en mangler host. "
                f"URL={url!r}."
            )
        ...
        current_path_casefold = (
            current_path.casefold()
        )

        context_path_casefold = (
            KY_CONTEXT_PATH.casefold()
        )

        already_correct = (
            current_path_casefold
            == context_path_casefold
            or current_path_casefold.startswith(
                context_path_casefold + "/"
            )
        )

        if already_correct:
            corrected_path = current_path
        else:
            corrected_path = (
                KY_CONTEXT_PATH
                + current_path
    ...
    def _build_task_url(
        ...
        """Tilføj ``opgaveId`` til URL'en uden at navigere.

        Hvis URL'en allerede indeholder ``opgaveId``, returneres URL'en
        uændret.

        Hvis URL'en allerede har en querydel, anvendes ``&``.
        Ellers anvendes ``?``.
        """
        ...
        if not url:
            raise ValueError(
                "original_url må ikke være tom."
            )

        if not task_id:
            raise ValueError(
                "task_id må ikke være tom."
            )
    ...
    @staticmethod
    def _package_pattern(
        package_name: str,
    ) -> re.Pattern:

        escaped = re.escape(
            OpgaveindbakkeClient._normalise(
                package_name
            )
        )
    ...
    def _extract_count(
        ...
        """Hent det afsluttende antal, eksempelvis ``(12)`` til ``12``."""
